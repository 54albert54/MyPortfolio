...
export const ProyecCarsdCover = ()=>{
  const { hiddeMoProyect }= useLandingContext()
 
  return(
  <section onClick={()=>hiddeMoProyect()} className="fixed w-screen h-screen right-0 z-10 bg-black/50 ">
...
>
      ...
      <div className="w-full h-auto border border-transparent flex justify-end items-center border-b-gray-900">
        <div onClick={()=> hiddeMoProyect()} className="hover:cursor-pointer border w-12 h-12 rounded-xl  bg-black/50  flex justify-center items-center my-2">
       ...
       <section className=" flex flex-col gap-y-12 pt-2">
        ...
        <ShowCode className="bg-gray-800/70 text-gray-100  shadow-lg font-bold" codeList={data?.code} />
...
const ShowLinks = ({data})=>(
  <div className="flex flex-col gap-4 py-5 pl-6">
    <div>
      ...
      <a className="parent  flex pl-2 flex-row items-center  justify-start gap-x-12 hover:text-blue-800" href={data?.pageLink}>
        <p className="text-ls font-light text-gray-400 text-xs">{data?.pageLink}</p> 
    ...
    <div className="">
      ...
      <a className="parent   flex pl-2 flex-row items-center  justify-start gap-x-12 hover:text-[24px]" href={data?.repoLink}>
        <p className="text-ls font-light text-slate-400 text-xs">{data?.repoLink}</p> 
        <FaArrowRight className="hidden-paragraph text-slate-400  w-6 h-6" />
      </a>
    </div>
